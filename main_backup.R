...
for(decomp in 1:length(Decomposed)) #Variação do número de métodos de decomposição
{  
  Data_Decomp<-Decomposed[[decomp]]
  MDecomp    <-ifelse(decomp==1,"EEMD","None")
  
  for(s in 1:length(Data_Decomp)) #Variação pro número de Datasets
  {
    ...
    for(i in 1:dim(data_dec)[2]) #Variação pro número de componentes
    {
      ...
      for(Horizonte in 1:2)
      {
        ...
        if(Horizon==7)   #7-steps-ahead
        {
          ...
          {
            #---------------------------OSA----------------------------------------------#
            PTRosa  <-matrix(nrow=cut,ncol=length(models));    #Recebe predições OSA das componentes
            PTEosa  <-matrix(nrow=n-cut,ncol=length(models));  #Recebe predições OSA das componentes
            
            PTrOSA <-matrix(nrow=cut,ncol=dim(combs)[1]+length(models)+1);     #Recebe predições OSA Combinadas + ARIMA model
            ETrOSA <-matrix(nrow=cut ,ncol=dim(combs)[1]+length(models)+1);    #Recebe erros OSA Combinadas     + ARIMA model
            PTeOSA <-matrix(nrow=n-cut ,ncol=dim(combs)[1]+length(models)+1);  #Recebe predições OSA Combinadas + ARIMA model
        ...
        else             #14-steps-ahead
        {
          ...
          {
            #-------------------------DSA------------------------------------------------#
            PTRdsa  <-matrix(nrow=cut,ncol=length(models));    #Recebe predições DSA das componentes
            PTEdsa  <-matrix(nrow=n-cut,ncol=length(models));  #Recebe predições DSA das componentes
            
            PTrDSA <-matrix(nrow=cut,ncol=dim(combs)[1]+length(models)+1);     #Recebe predições DSA Combinadas 
            ETrDSA <-matrix(nrow=cut ,ncol=dim(combs)[1]+length(models)+1);    #Recebe erros DSA Combinadas 
            PTeDSA <-matrix(nrow=n-cut ,ncol=dim(combs)[1]+length(models)+1);  #Recebe predições DSA Combinadas 
    ...
    #----------------------------Combinação-----------------------------------#
    for(c in 1:dim(combs)[1]) #Variação pro número de combinações
    {
    ...
    for(m in 1:(length(models)+1))
    {
      ###################################Performance#####################################
      {
        ...
        ETeDSA[,c+m]<-as.vector(Output[(cut+15):length(Output)]-PTeDSA[1:(n-cut),c+m])
          ...
          #--------------------Performance Measures---------------------#
         POSA[c+m,1:4]<-PM(Output[15:(cut+14)]  ,PTrOSA[1:cut,c+m]) #O 14 se refere ao númerode lags adotados
         ...
         PDSA[c+m,1:4]<-PM(Output[15:(cut+14)]  ,PTrDSA[1:cut,c+m]) #O 14 se refere ao númerode lags adotados
...
ggplot(aes(reorder(variables, Average), Average)) + 
  ...
  scale_fill_gradient2(low = "yellow", 
                       high = "red") + 
  scale_x_discrete(labels = c(expression(Y[t-1]),
